...
jQuery(document).ready(function() {
    ...
    $('a[href="#MySearch"]').on('click', function(event) {
        event.preventDefault();
        $('#MySearch').addClass('open');
        $('#MySearch > form > input[type="search"]').focus();
    });
    //fermeture btn-close modal search
    $('#MySearch, #MySearch button.close').on('click', function(event) {
	...
	if (jQuery().owlCarousel) {
        ...
        //carousel du blog
        var $fromBlog = jQuery("#from-blog");
        var shownBlogItems = ($fromBlog.hasClass('classic-blog-items')) ? 1 : 3;
        $fromBlog.owlCarousel({
            navigation : true,
            pagination : false,
            ...
            + $fromBlog.data('prev-text')
            + '</a>',
                '<a href="#">'
                + $fromBlog.data('next-text')
                + '</a>']
...
jQuery(window).load(function(){
	...
	jQuery(".preloader").delay(500).fadeOut("slow").delay(600, function(){
		jQuery(this).remove();
	});
	...
	if (jQuery().jflickrfeed) {
		jQuery("#flickr").jflickrfeed({
			...
			itemTemplate: '<a href="{{image_b}}" data-gal="prettyPhoto[pp_gal]"><li><images alt="{{title}}" src="{{image_s}}" /></li></a>'
		}, function(data) {
			jQuery("#flickr a").prettyPhoto({
			   ...
			   jQuery(this).find("images").stop().animate({ opacity: 0.5 }, 200);
		    }, function() {
			   jQuery(this).find("images").stop().animate({ opacity: 1.0 }, 400);
		    });
		});
	}
